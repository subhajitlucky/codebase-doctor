...
export interface LoadCatalogOptions {
  ...
  clientFactory?: (config: CatalogClientConfig) => CatalogClient;
}
...
export interface CatalogClient {
  ...
  query<Row>(statement: string, values?: readonly unknown[]): Promise<{ rows: Row[] }>;
  end(): Promise<void>;
}
...
export async function loadCatalog(options: LoadCatalogOptions): Promise<CatalogSnapshot> {
  const config: CatalogClientConfig = {
    ...
    application_name: "codebase-doctor"
  };
  const client = options.clientFactory?.(config) ?? new Client(config) as unknown as CatalogClient;

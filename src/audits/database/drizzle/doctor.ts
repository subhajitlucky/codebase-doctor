...
function findingFor(
  ...
  return {
    ...
    remediation:
      "Have an authorized human or external coding agent use a typed Drizzle comparison such as lte(column, date), or supply an explicit encoder; only a fresh inline callable encoder is statically recognized as safe. Then rerun the audit.",
    verification: {
      command: changed
